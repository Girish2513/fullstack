...
async function getGithubUser(username) {
    try {
        const response = await axios_1.default.get(`${GITHUB_API}/users/${username}`, {
            timeout: 5000,
        });
        ...
        if (err instanceof axios_1.AxiosError && err.response) {
            throw new ExternalApiError(err.response.data?.message || err.message, err.response.status);
        }
        throw err;
    }
}
